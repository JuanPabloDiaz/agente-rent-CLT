...
Apps Script also runs a **weekly seed snapshot** in the reverse direction (Sheet → Gmail → 2BR agent): `snapshotAptoCltForCrossAgent()` filters the `1 bed` tab by STATUS (only rows in `SEED_INCLUDE` — pre-visit signals `LOVE` / `LGTM` / `Need 2 Go!` / `Maybe` / `Missing` plus post-visit signals `Fui - LGTM` / `Fui - LOVE` / `Fui - $$$ LOVE`; all `NO - *` variants and blanks are dropped) and emails a JSON block (subject `APTO-CLT-SEEDS weekly`, markers `<<<APTO-CLT-SEEDS-START>>>` / `<<<APTO-CLT-SEEDS-END>>>`, payload version `2` with a single `seeds` array) that the `apto-2bed-2bath` agent reads via Gmail MCP `search_threads` to reuse prior human triage. This thread is **not** consumed by `pollGmailDrafts` — its subject deliberately does not match any agent's `subjectPrefix`. STATUS filter mapping lives in the `SEED_INCLUDE` set at the top of `Code.gs`.
...
- **`income`** — active, **on-demand** (not cron). Paystub tracker. User drops a payroll-portal screenshot (Costco, bi-weekly) into Claude Code and references `agents/income/prompt.md`; the agent extracts every visible paystub, dedupes by `LINK=paystub-<PAY_DATE>` against prior Gmail digests, and emits a single Gmail draft with the JSON payload. Rows land on the `INCOME` tab of the shared apto-clt spreadsheet. A separate `income-summary` tab holds formulas (last-4 totals, last-2-months window, YTD, annualized × 26/4) that recompute automatically as new paystubs land. HTML digest columns are overridden per-agent via `htmlColumns` on the `AGENTS[]` entry so the emailed table shows income-relevant columns instead of listing columns. No update-in-place semantics — paystub corrections require manually editing the sheet row.
